...
create table t5_users_reviews_comments(
        ...
        id int(11) auto_increment,
        user_id int(11),
        user_reviews_id int(11),
        comment text,
                
        dated varchar(20),
        primary key(id),
    foreign key (user_id) references t5_users(id) on update cascade on delete cascade,
        foreign key (user_reviews_id) references t5_users_reviews(id) on update cascade on delete cascade
...
create table t5_users_votes(
        ...
        primary key(id),
        unique key(user_id,user_reviews_id),
    foreign key (user_id) references t5_users(id) on update cascade on delete cascade,
        foreign key (user_reviews_id) references t5_users_reviews(id) on update cascade on delete cascade
